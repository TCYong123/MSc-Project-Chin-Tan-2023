...
def main(raw_args=None, mesh=None, Gam=None):
    ...
    u_out = fd.Function(V1DG, name="u_outTR")
    h_out = fd.Function(V2, name="h_outTR")
    q_out = fd.Function(V0, name="q_outTR")
    b_out = fd.Function(V2, name="b_outTR")
    ...
    while t < tmax + 0.5*dt:
        ...
        if args.energy == 1:
            np.savetxt("trR_benergy"+str(dt)+"_"+str(dmax)+".array", energy_t)
        if args.energy == 2:
            np.savetxt("trR_benergy"+str(nrefs)+"_"+str(dt)+"_"+str(dmax)+".array", energy_t)    

        if args.iter == 1:
            np.savetxt("trR_stepcount_"+str(dt)+"_"+str(dmax)+".array", stepcount_array)  
            np.savetxt("trR_itcount_"+str(dt)+"_"+str(dmax)+".array", itcount_array)               
       
        if args.vorticity == 1:
            vorticity_t = np.append(vorticity_t, qn.dat.data)  
            np.savetxt("trR_vorticity"+str(dt)+"_"+str(dmax)+".array", vorticity_t)
        if args.vorticity == 2:
            vorticity_t = np.append(vorticity_t, qn.dat.data)  
            np.savetxt("trR_vorticity"+str(nrefs)+str(dt)+"_"+str(dmax)+".array", vorticity_t)
