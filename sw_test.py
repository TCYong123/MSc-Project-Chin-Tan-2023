...
def main(raw_args=None):
    ...
    uI = fd.Function(V1DG, name="uI")
    hI = fd.Function(V2, name="hI")
    ...
    V1DG = fd.VectorFunctionSpace(mesh,"DG",degree+1)
    uT = fd.Function(V1DG, name="uT")
    hT = fd.Function(V2, name="hT")
    ...
    uT.interpolate(u0)
    hT.interpolate(h0)
    with fd.CheckpointFile("test_dt"+str(dt)+".h5", 'w') as afile:
        afile.save_mesh(mesh)  
        afile.save_function(uI)
        afile.save_function(hI)
        afile.save_function(uT)
        afile.save_function(hT)        
