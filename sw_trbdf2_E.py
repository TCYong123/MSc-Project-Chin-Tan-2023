...
def main(raw_args=None, mesh=None, Gam=None):
    ...
    if args.solver_mode == 'AL':
        ...
        sparameters = {
            "snes_monitor": None,
            "snes_ksp_ew": None, #enables Eisenstat-Walker
            "mat_type": "matfree",
            "ksp_type": "fgmres",
    ...
    while t < tmax + 0.5*dt:
        ...
        if args.energy == 1:
            np.savetxt("trE_benergy"+str(dt)+"_"+str(dmax)+".array", energy_t)
        if args.energy == 2:
            np.savetxt("trE_benergy"+str(nrefs)+"_"+str(dt)+"_"+str(dmax)+".array", energy_t)
        ...
        if args.vorticity == 1:
            ...
            np.savetxt("trE_vorticity"+str(dt)+"_"+str(dmax)+".array", vorticity_t)
        if args.vorticity == 2:
            vorticity_t = np.append(vorticity_t, qn.dat.data)  
            np.savetxt("trE_vorticity"+str(nrefs)+str(dt)+"_"+str(dmax)+".array", vorticity_t)
