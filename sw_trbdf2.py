...
def main(raw_args=None, mesh=None, Gam=None):
    ...
    while t < tmax + 0.5*dt:
        ...
        if args.energy == 1:
            np.savetxt("tr_benergy"+str(dt)+"_"+str(dmax)+".array", energy_t)
        if args.energy == 2:
            np.savetxt("tr_benergy"+str(nrefs)+"_"+str(dt)+"_"+str(dmax)+".array", energy_t)
        ...
        if args.vorticity == 1:
            ...
            np.savetxt("tr_vorticity"+str(dt)+"_"+str(dmax)+".array", vorticity_t)
        if args.vorticity == 2:
            vorticity_t = np.append(vorticity_t, qn.dat.data)  
            np.savetxt("tr_vorticity"+str(nrefs)+str(dt)+"_"+str(dmax)+".array", vorticity_t)
